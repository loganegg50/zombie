...
/** 경사로 나뭇잎 — 상단부에만 배치. [수평 투영, Z오프셋, 반지름] */
const RAMP_LEAF_DATA: [number, number, number][] = [
  [3.0,  0.4, 0.9],
  [3.6, -0.4, 1.0],
  [4.2,  0.3, 0.85],
];

/** 월드 좌표 나뭇잎 구체 [x, y, z, 반지름] — 스텔스 판정용 */
export const RAMP_LEAF_SPHERES: [number, number, number, number][] =
  RAMP_LEAF_DATA.map(([proj, zOff, r]) => {
...
export class SceneManager {
  ...
  private buildDecorations(): void {
    ...
    const darkTrunkMat = new THREE.MeshStandardMaterial({ color: 0x5a3520 });
    const leafMats = [
      new THREE.MeshStandardMaterial({ color: 0x2d8a4e }),
      new THREE.MeshStandardMaterial({ color: 0x238b3a }),
      new THREE.MeshStandardMaterial({ color: 0x3a9d5c }),
      new THREE.MeshStandardMaterial({ color: 0x1e7a3a }),
    ];

    // 나무별 크기/형태 변형
    const treeVariants: { scale: number; canopies: [number, number, number, number][]; branches: number }[] = [
      { scale: 1.0, canopies: [[0, 3.0, 0, 1.3], [0.5, 3.5, 0.3, 1.0], [-0.4, 3.8, -0.2, 0.9], [0.1, 4.2, 0.1, 0.7]], branches: 2 },
      { scale: 0.85, canopies: [[0, 2.6, 0, 1.1], [0.3, 3.0, 0.2, 0.9], [-0.3, 3.2, -0.3, 0.85], [0, 3.5, 0, 0.6]], branches: 1 },
      { scale: 1.15, canopies: [[0, 3.2, 0, 1.5], [0.6, 3.8, 0.4, 1.1], [-0.5, 3.6, -0.3, 1.2], [0.2, 4.5, -0.2, 0.9], [-0.3, 4.8, 0.3, 0.7]], branches: 3 },
      { scale: 0.9, canopies: [[0, 2.8, 0, 1.2], [0.4, 3.3, -0.2, 0.95], [-0.2, 3.6, 0.4, 0.8]], branches: 1 },
      { scale: 1.05, canopies: [[0, 3.1, 0, 1.4], [-0.5, 3.5, 0.3, 1.0], [0.4, 3.9, -0.3, 1.1], [0, 4.3, 0, 0.8], [0.3, 4.6, 0.2, 0.6]], branches: 2 },
    ];

    // 일반 나무
    for (let ti = 0; ti < TREE_POSITIONS.length; ti++) {
      const [x, z] = TREE_POSITIONS[ti];
      const variant = treeVariants[ti % treeVariants.length];
      const s = variant.scale;

      // 몸통 (약간 기울어진 느낌)
      const trunkH = 2.5 * s;
      const trunk = new THREE.Mesh(
        new THREE.CylinderGeometry(0.15 * s, 0.3 * s, trunkH, 8),
        trunkMat,
      );
      ...
      // 뿌리 돌출 (3개)
      for (let r = 0; r < 3; r++) {
        const angle = (r / 3) * Math.PI * 2 + ti * 0.7;
        const root = new THREE.Mesh(
          new THREE.CylinderGeometry(0.04 * s, 0.1 * s, 0.6 * s, 4),
          darkTrunkMat,
        );
        root.position.set(
          x + Math.cos(angle) * 0.2 * s,
          0.15 * s,
          z + Math.sin(angle) * 0.2 * s,
        );
      ...
      // 가지
      for (let b = 0; b < variant.branches; b++) {
        const angle = (b / variant.branches) * Math.PI * 2 + ti * 1.2;
        const branchLen = (0.8 + b * 0.3) * s;
        const branchY = (1.8 + b * 0.6) * s;
        const branch = new THREE.Mesh(
          new THREE.CylinderGeometry(0.03 * s, 0.08 * s, branchLen, 5),
          darkTrunkMat,
        );
        branch.position.set(
          x + Math.cos(angle) * branchLen * 0.35,
          branchY,
          z + Math.sin(angle) * branchLen * 0.35,
        );
      ...
      // 여러 겹 나뭇잎 구체
      for (let ci = 0; ci < variant.canopies.length; ci++) {
        const [cx, cy, cz, cr] = variant.canopies[ci];
        const mat = leafMats[(ti + ci) % leafMats.length];
        const canopy = new THREE.Mesh(
          new THREE.SphereGeometry(cr * s, 7, 6),
          mat,
        );
        canopy.position.set(x + cx * s, cy * s, z + cz * s);
